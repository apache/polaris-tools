      ...
      if (project != project.rootProject) {
        ...
        task.doFirst {
          mavenPom.run {
            ...
            val codeRepo: Provider<String> = e.overrideScm.orElse(
              githubRepoName.map { r -> "https://github.com/apache/$r" }.orElse(projectPeople.repository))
            ...
            issueManagement {
              val issuesUrl: Provider<String> = codeRepo.map { r -> "$r/issues" }.orElse(projectPeople.bugDatabase)
...
fun addContributorsToPom(mavenPom: MavenPom, githubRepoName: String, asfProjectName: String) =
  mavenPom.run {
    contributors {
      val contributors: List<Map<String, Any>> =
        parseJson("https://api.github.com/repos/apache/$githubRepoName/contributors?per_page=1000")
      contributors
        ...
        .forEach { contributor ->
          contributor {
            ...
            organizationUrl.set("https://github.com/apache/$githubRepoName")
          }
        }
    }
  }
