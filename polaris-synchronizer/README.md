...
* `--credential-output-type`: One of `CONSOLE` (default), `FILE`, or `CUSTOM`.
  ...
  * `CUSTOM`: Loads a user-supplied class implementing `CredentialWriter`, allowing you to plug in your own storage
    backend (e.g. a secrets manager) without modifying this tool.
* `--credential-output-properties`: Properties to configure the selected type.
  ...
  * For `CUSTOM`:
    * `custom-impl`: the fully-qualified classname of your `CredentialWriter` implementation.
