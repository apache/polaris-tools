...
class PolarisRestToolCallTests(unittest.TestCase):
    def _create_tool(self) -> tuple[PolarisRestTool, mock.Mock, mock.Mock]:
        http = mock.Mock()
        auth = mock.Mock()
    ...
    def test_call_builds_request_and_metadata_with_json_body(self) -> None:
        tool, http, auth = self._create_tool()
        ...
        self.assertIn("POST " + expected_url, result.text)
        self.assertIn('"result": "ok"', result.text)
        self.assertEqual(result.metadata["method"], "POST")
        self.assertEqual(result.metadata["url"], expected_url)
        self.assertEqual(result.metadata["status"], 201)
        self.assertEqual(result.metadata["request"]["body"], {"name": "analytics"})
        self.assertEqual(result.metadata["response"]["body"], {"result": "ok"})
        self.assertEqual(result.metadata["response"]["headers"]["X-Request-Id"], "abc123")
        self.assertEqual(result.metadata["request"]["headers"]["Authorization"], "[REDACTED]")

    def test_call_uses_authorization_provider_and_handles_plain_text(self) -> None:
        tool, http, auth = self._create_tool()
        ...
        self.assertIn("Status: 404", result.text)
        self.assertEqual(result.metadata["url"], expected_url)
        self.assertEqual(result.metadata["request"]["bodyText"], "payload")
        self.assertEqual(result.metadata["response"]["bodyText"], "failure")
        self.assertEqual(result.metadata["response"]["headers"]["X-Trace"], "abc, def")
        self.assertEqual(result.metadata["request"]["headers"]["Authorization"], "[REDACTED]")

    def test_call_requires_non_empty_path(self) -> None:
        tool, http, _ = self._create_tool()
